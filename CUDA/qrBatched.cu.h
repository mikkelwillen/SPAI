...
__global__ void tauDevicePointerToDeviceKernel(float** d_tau, float* h_tau, int batch, int n1, int n2) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
int qrBatched(float* AHat, int n1, int n2, float* Q, float* R) {
    ...
    tauDeviceToDevicePointerKernel <<< 1, BATCHSIZE * n1T >>> (d_PointerTau, d_tau, BATCHSIZE, n1T);
